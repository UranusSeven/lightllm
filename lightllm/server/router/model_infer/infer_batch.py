...
class InferBatch:
    ...
    def init_batch(
        ...
        nopad_b_loc = torch.empty(
            (len(requests), setting["max_req_total_len"] + 12),
            dtype=torch.long,
            device="cuda",
        )
        ...
        nopad_b_start_loc[1:] = torch.cumsum(nopad_b_seq_len, dim=0, dtype=torch.int32)[
            0:-1
        ]
        if len(requests) > 1:
    ...
    def filter(self, request_ids: List[int]):
        ...
        nopad_max_len_in_batch = 0
        nopad_b_loc = torch.empty(
            (len(request_ids), setting["max_req_total_len"] + 12),
            dtype=torch.long,
            device="cuda",
        )
        ...
        nopad_b_loc[:, 0 : (nopad_max_len_in_batch - 1)] = self.nopad_b_loc[
            indices,
            (self.nopad_max_len_in_batch - 1)
            - (nopad_max_len_in_batch - 1) : (self.nopad_max_len_in_batch - 1),
        ]
        for i, request_id in enumerate(request_ids):
            idx = self.requests_idx_mapping[request_id]
    ...
    def merge(cls, batch1, batch2):
        ...
        nopad_total_token_num = (
            batch1.nopad_total_token_num + batch2.nopad_total_token_num
        )
        nopad_max_len_in_batch = max(
            batch1.nopad_max_len_in_batch, batch2.nopad_max_len_in_batch
        )

        nopad_b_loc = torch.empty(
            (new_batch_size, setting["max_req_total_len"] + 12),
            dtype=torch.long,
            device="cuda",
        )
        nopad_b_start_loc = torch.zeros(
        ...
        for i, batch in enumerate(batches):
            ...
            nopad_b_start_loc[start_index:end_index] = (
                batch.nopad_b_start_loc + nopad_start_loc_len_temp
            )
            nopad_start_loc_len_temp = (
                nopad_b_start_loc[end_index - 1] + nopad_b_seq_len[end_index - 1]
            )
            nopad_b_loc[
                start_index:end_index,
                nopad_max_len_in_batch
                - batch.nopad_max_len_in_batch : nopad_max_len_in_batch
                - 1,
        ...
        nopad_b_loc[:, nopad_max_len_in_batch - 1] = (
            nopad_total_token_num
            - new_batch_size
            + torch.arange(0, new_batch_size, dtype=torch.int32, device="cuda")
        )
        return InferBatch(
