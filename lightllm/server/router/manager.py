...
class RouterManager:
    def __init__(
        ...
        self.req_queue = ReqQueue(
            max_total_token_num, batch_max_tokens, running_max_req_size
        )
    ...
    def add_req(
        self, prompt_ids: List[int], sampling_params: SamplingParams, request_id: str
    ):
            ...
            if self.running_batch is not None:
                if counter_count % 50 == 0:
                    print(
                        ...
                        "token used ratio:",
                        self.running_batch.calcu_used_tokens()
                        / self.max_total_token_num,
                    )
                    pass
    ...
    async def _filter_batch(self, batch: Batch):
        ...
        rets = [
            self.model_rpcs[tp_rank].filter_batch(batch.batch_id, req_id_list)
            for tp_rank in range(self.world_size)
        ]
        await asyncio.gather(*rets)
        return
