...
def test2():
    ...
    Prob = torch.empty((H, B * N_CTX), dtype=dtype, device="cuda").normal_(
        mean=0.4, std=0.2
    )
    ...
    for i in range(0, B):
        ...
        b_loc[i, N_CTX - b_seq_len[i] :] = b_start_loc[i] + torch.arange(
            0, b_seq_len[i], dtype=torch.int32, device="cuda"
        )
