...
def token_attention_fwd(
    q, k, v, o, alibi, b_loc, b_start_loc, b_seq_len, max_len_in_batch
):
    head_num = k.shape[1]
    ...
    att_m_tensor = None
    token_att_fwd2(
        prob, v, o.view(calcu_shape1), b_loc, b_start_loc, b_seq_len, max_len_in_batch
    )
    prob = None
    return
...
def test():
    ...
    dtype = torch.float16
    q = torch.empty((Z, H, D_HEAD), dtype=dtype, device="cuda").normal_(
        mean=0.1, std=0.2
    )
    ...
    o = torch.empty((Z, H, D_HEAD), dtype=dtype, device="cuda").normal_(
        mean=0.3, std=0.2
    )
