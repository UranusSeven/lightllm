...
def token_att_fwd_int8k(
    q, k, k_scale, att_out, B_Loc, B_Start_Loc, B_Seqlen, max_input_len
):
    BLOCK = 32
    # shape constraints
    Lq, Lk = q.shape[-1], k.shape[-1]
...
def test1():
    ...
    k = torch.empty((B * N_CTX, H, D), dtype=dtype, device="cuda").normal_(
        mean=0.1, std=0.2
    )
...
def test2():
    ...
    k_scale = k.abs().max(-1, keepdim=True)[0] / 127.0
    ...
    for _ in range(10):
        token_att_fwd_int8k(
            q, int_k, k_scale, att_out, b_loc, b_start_loc, b_seq_len, N_CTX
        )
    run_iter = 1000
    torch.cuda.synchronize()
    t1 = time.time()
