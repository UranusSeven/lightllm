...
def _fwd_kernel_token_att2(
    ...
    v_loc_off = (
        cur_batch * stride_b_loc_b + (cur_batch_start_index + offs_n) * stride_b_loc_s
    )
    ...
    for start_n in range(0, cur_batch_seq_len, BLOCK_N):
        ...
        p_value = tl.load(
            Prob + p_offs + start_n * stride_b_loc_s,
            mask=(start_n + offs_n) < cur_batch_seq_len,
            other=0.0,
        )
        v_loc = tl.load(
            B_Loc + v_loc_off + start_n * stride_b_loc_s,
            mask=(start_n + offs_n) < cur_batch_seq_len,
            other=0.0,
        )
        v_value = tl.load(
...
def test1():
    ...
    V = torch.empty((B * N_CTX, H, D), dtype=dtype, device="cuda").normal_(
        mean=0.1, std=10
    )
    Prob = (
        torch.empty((H, B * N_CTX), dtype=dtype, device="cuda")
...
def test2():
    ...
    V_scale = V.abs().max(-1, keepdim=True)[0] / 127.0
    ...
    for _ in range(10):
        token_att_fwd2_int8v(
            Prob, int_V, V_scale, Out, b_loc, b_start_loc, b_seq_len, N_CTX
        )
    ...
    print(
        "cos ",
        cos(torch_out.flatten().to(torch.float32), o.flatten().to(torch.float32)),
    )
